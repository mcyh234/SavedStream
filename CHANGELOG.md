...
- 广场、我的公开和我的点赞不渲染文件夹；普通用户的文件夹 API 只返回包含其私人媒体的文件夹及必要祖先目录，不再暴露其他用户私人文件夹的名称。
- 进入任意文件夹后仍可直接新建子文件夹，创建请求使用当前文件夹作为 `parent_id`，面包屑可逐级返回。
- 媒体列表新增标题、类型、大小、日期四种排序字段以及升序/降序组合；后端使用包含排序值、账号和消息 ID 的稳定游标，加载更多时不会因同值项目出现重复或遗漏。
- 列表“快速操作”加入下载图标，普通用户与管理员均可直接启动现有加密分块下载；管理员继续同时看到可见性和删除操作。
...
- 管理员门户登录优先使用对应的门户用户身份，点赞和站内信不再因恢复密钥会话缺少用户 ID 而返回 401。
- 恢复密钥会话使用隐藏的系统管理员身份承载个人互动数据，不出现在普通用户管理列表中；管理员可在广场正常点赞，个人信箱请求也保持一致。

## Telegram 多账号容灾

- 新增逻辑账号组、主账号/备用账号角色、首次历史回填、指纹去重和异步实时复制。
- 新增健康检查与连续三次失败自动故障切换；管理员后台显示同步进度、积压和错误。
- 增加复制重试恢复、Telegram marker 幂等扫描、Moderation mutation 同步、点赞迁移与活动账号不可用错误提示。

## 文件夹上传与上传反馈优化

- 在文件夹界面拖拽或选择文件时，上传任务记录当前文件夹，Telegram 入库和索引完成后自动移动到该文件夹。
- 文件移入文件夹后从根级“全部文件”隐藏；根级搜索会自动展开并返回文件夹中的匹配资源。文件再次移动时以新文件夹替换旧位置。
- WebUI 和管理员兼容上传入口不再提供物理账号选择，服务端根据用户绑定、逻辑账号组和当前活动账号自动路由。
- “广场”界面上传默认选择公开；普通用户仍以私人状态入库并进入管理员审核，管理员公开上传直接通过。
- 批次全部成功后，上传行逐项平滑收起并显示绿色完成图标，5 秒后自动关闭；失败/取消项显示红色错误图标并保留重试入口。
...
- 新表 `media_folders`（`parent_id` 树形、同级名称唯一）与 `media_folder_items`（每个文件保存一个当前文件夹位置）。
...
- 普通用户按自己的逻辑绑定上传；管理员和普通用户均由服务端自动选择当前活动物理账号。普通用户公开上传先以私人状态写入并进入审核，管理员公开上传直接公开。
