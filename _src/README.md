...
文件夹作为置顶条目与媒体共用同一个网格/列表，进入文件夹后可继续创建子文件夹。媒体 API 支持按标题、类型、大小、日期升序/降序稳定分页，列表快速操作栏可直接启动加密下载。

管理员可在后台“安全策略”上传多个逐行 TXT 文件名敏感词库。上传前会对 WebUI、Helper Bot 和 Bridge 的文件名做大小写不敏感匹配；管理员修改本地标题时也会执行相同检查。命中返回结构化错误，重复命名攻击按持久化 10 分钟窗口限流。

`/bind` 邀请码支持管理员开关和限额：管理员可设置 24 小时全局新绑定数以及每个用户 24 小时邀请码生成数。普通用户可从网页钥匙按钮或 Helper Bot `/bind`（无参数）生成邀请码，其他用户使用 `/bind <邀请码>` 绑定；管理员创建邀请码仍保持独立的管理员接口。
