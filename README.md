...
- **管理后台 15 个页签**：仪表盘 / 多账号 / 审核 / 举报受理 / 用户 / 公开相册 / 媒体库 / 上传 / 流量 / 缓存 / Bot 限流 / 信箱 / 备份 / 安全策略 / 存储，配置保存按钮右侧悬浮常驻
...
- **文件名敏感词防护**：管理员可上传多个逐行 TXT 词库；WebUI、Helper Bot 和 Telegram Bridge 上传前检查文件名，管理员改名同样检查。命中会拒绝操作，并对连续命名攻击进行持久化限流。
- **/bind 邀请码策略**：管理员可开关普通用户生成邀请码，并设置 24 小时全局新绑定数和单用户生成数；普通用户可在网页或 Helper Bot 发送 `/bind`（无参数）生成邀请码，再由目标用户使用 `/bind <邀请码>`。
