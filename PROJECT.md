...
> **快速迭代提示：** 当前程序处于快速迭代期，使用可能会出现体验不佳或各种 Bug，可以提交 Issue 反馈。

## Telegram 多账号容灾（快速迭代）

SavedStream 支持逻辑账号组、主账号与多个备用容灾账号。新增副本后会从最早媒体开始同步，默认每 3 秒最多复制一条、每分钟不超过 10 条；管理员可以在安全下限内调整策略。WebUI/Helper Bot 新文件采用异步耐久复制，主流程不等待副本完成。主账号连续三次健康检查失败时自动切换到同步完成且优先级最高的副本，恢复后不自动切回。

## 文件名策略与 `/bind` 邀请码

管理后台“安全策略”支持上传多个 UTF-8 TXT 敏感词库（每行一个词）。词库在上传后立即生效：WebUI、Helper Bot 和 Bridge 会在文件入库前检查原始文件名，管理员修改本地标题时也会检查；命名命中会被拒绝，连续命中尝试按 10 分钟窗口持久化限流。管理员可调整命中次数和冷却时间。

同一页面可开关普通用户生成 `/bind` 邀请码，并设置滚动 24 小时全局新绑定上限与单用户邀请码生成上限。管理员生成的邀请码不受普通用户生成上限影响；用户可在网页点击钥匙按钮或在 Helper Bot 私聊发送 `/bind`（无参数）生成邀请码，接收方使用 `/bind <邀请码>` 完成绑定。
...
- **前端交互（2026-08-30 调整）**：文件夹不再使用独立容器，而是作为置顶条目与普通文件共用网格/列表；进入任意文件夹后，“新建文件夹”自动使用当前目录作为 `parent_id`。标题上方有完整路径面包屑（全部文件 / 上级 / 当前文件夹），每一级可点击跳回；管理员可重命名或删除文件夹（递归删除子树），批量工具栏保留“移动到文件夹”。侧边栏只保留媒体分类。
- 媒体列表支持 `sort=title|kind|size|date` 与 `direction=asc|desc`。非默认排序采用包含排序值、物理账号与消息 ID 的版本化游标，保证跨账号和相同排序值下的稳定分页；旧 `order=newest|oldest` 调用继续兼容。
- 列表快速操作列对所有可访问媒体显示加密下载按钮；管理员在同一列继续使用可见性选择和删除按钮。
